...
public class InsertDaoImpl implements InsertDao{
	...
	PreparedStatement ps=null;
		try {
			String query="insert into student99(username,password,email,contact,age) values(?,?,?,?,?)";
	...
	public List<UserBean> getValues() {
		...
		ResultSet rs=null;
		try {
			String query="select * from student99";
			conn=DbConnection.get_connection();
			ps=conn.prepareStatement(query);
			rs=ps.executeQuery();
			UserBean ubean=null;
			while(rs.next()){
				...
				ubean.setUname(rs.getString("username"));
				  ...
				  ubean.setPwd(rs.getString("password"));
	...
	public void delete(UserBean userBean) {
		...
		try {
			...
			String sql="delete from student99 where id=?";
	...
	public void updateValues(UserBean userBean) {
		...
		try {
			connection=DbConnection.get_connection();
			String sql="update student99 set username =?,password=?,email=?,contact=?,age=? where id=?";
			preparedStatement=connection.prepareStatement(sql);
